  ...
  module Fusion
    module ElasticLoadBalancer
      ...
      def register_instance(env, elb_name, instance_id)
        env[:ui].info I18n.t("vagrant_fusion.elb.registering", instance_id: instance_id, elb_name: elb_name), :new_line => false
        elb = get_load_balancer(env[:fusion_elb], elb_name)
        unless elb.instances.include? instance_id
          elb.register_instances([instance_id])
          env[:ui].info I18n.t("vagrant_fusion.elb.ok"), :prefix => false
          adjust_availability_zones env, elb
        else
          env[:ui].info I18n.t("vagrant_fusion.elb.skipped"), :prefix => false
        end
      end

      def deregister_instance(env, elb_name, instance_id)
        env[:ui].info I18n.t("vagrant_fusion.elb.deregistering", instance_id: instance_id, elb_name: elb_name), :new_line => false
        elb = get_load_balancer(env[:fusion_elb], elb_name)
      ...
      def adjust_availability_zones(env, elb)
        env[:ui].info I18n.t("vagrant_fusion.elb.adjusting", elb_name: elb.id), :new_line => false

        instances = env[:fusion_compute].servers.all("instance-id" => elb.instances)
      ...
      def get_load_balancer(fusion, name)
        fusion.load_balancers.find { |lb| lb.id == name } or raise Errors::ElbDoesNotExistError
      end
    end
  end
end
