...
module VagrantPlugins
  module Fusion
    module Action
      # This action packages a running fusion-based server into an
      # fusion-based vagrant box. It does so by burning the associated
      # vagrant-fusion server instance, into an AMI via fog. Upon
      ...
      class PackageInstance < Vagrant::Action::General::Package
        ...
        def initialize(app, env)
          super
          @logger = Log4r::Logger.new("vagrant_fusion::action::package_instance")
          env["package.include"] ||= []
        end
        ...
        def call(env)
          ...
          begin
            ...
            env[:ui].info(I18n.t("vagrant_fusion.packaging_instance", :instance_id => server.id))
            
            # Make the request to Fusion to create an AMI from machine's instance
            ...
            # Attempt to burn the fusion instance into an AMI within timeout
            env[:metrics]["instance_ready_time"] = Util::Timer.time do
              ...
              env[:ui].info(I18n.t("vagrant_fusion.burning_ami", :ami_id => @ami_id))
            ...
            env[:ui].info(I18n.t("vagrant_fusion.packaging_instance_complete", :time_seconds => env[:metrics]["instance_ready_time"].to_i))
        ...
        def create_vagrantfile env
          File.open(File.join(env["export.temp_dir"], "Vagrantfile"), "w") do |f|
            f.write(TemplateRenderer.render("vagrant-fusion_package_Vagrantfile", {
              region: env[:machine].provider_config.region,
              ami: @ami_id,
        ...
        # Used to find the base location of fusion-vagrant templates
        def template_root
          Fusion.source_root.join("templates")
        end
