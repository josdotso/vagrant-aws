    ...
    module Action
      ...
      class ReadSSHInfo
        def initialize(app, env)
          @app    = app
          @logger = Log4r::Logger.new("vagrant_fusion::action::read_ssh_info")
        end

        def call(env)
          env[:machine_ssh_info] = read_ssh_info(env[:fusion_compute], env[:machine])

          @app.call(env)
        end

        def read_ssh_info(fusion, machine)
          return nil if machine.id.nil?

          # Find the machine
          server = fusion.servers.get(machine.id)
          if server.nil?
