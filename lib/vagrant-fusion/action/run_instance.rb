...
require 'vagrant-fusion/util/timer'
    ...
    module Action
      ...
      class RunInstance
        ...
        def initialize(app, env)
          @app    = app
          @logger = Log4r::Logger.new("vagrant_fusion::action::run_instance")
        end
        ...
        def call(env)
          ...
          if !keypair
            env[:ui].warn(I18n.t("vagrant_fusion.launch_no_keypair"))
          end

          # If there is a subnet ID then warn the user
          if subnet_id && !elastic_ip
            env[:ui].warn(I18n.t("vagrant_fusion.launch_vpc_warning"))
          end

          # Launch!
          env[:ui].info(I18n.t("vagrant_fusion.launching_instance"))
          ...
          if !security_groups.empty?
            ...
            env[:ui].warn(I18n.t("vagrant_fusion.warn_ssh_access")) unless allows_ssh_port?(env, security_groups, subnet_id)
          end

          begin
            server = env[:fusion_compute].servers.create(options)
          rescue Fog::Compute::Fusion::NotFound => e
            ...
            raise
          rescue Fog::Compute::Fusion::Error => e
          ...
          env[:metrics]["instance_ready_time"] = Util::Timer.time do
            ...
            env[:ui].info(I18n.t("vagrant_fusion.waiting_for_ready"))
            begin
          ...
          if !source_dest_check.nil?
            if server.vpc_id.nil?
                env[:ui].warn(I18n.t("vagrant_fusion.source_dest_checks_no_vpc"))
            else
                begin
                    attrs = {
                        "SourceDestCheck.Value" => source_dest_check
                    }
                    env[:fusion_compute].modify_instance_attribute(server.id, attrs)
                rescue Fog::Compute::Fusion::Error => e
                    raise Errors::FogError, :message => e.message
                end
            end
          ...
          if !env[:interrupted]
            env[:metrics]["instance_ssh_time"] = Util::Timer.time do
              ...
              env[:ui].info(I18n.t("vagrant_fusion.waiting_for_ssh"))
              ...
              while true
                ...
                begin
                  ...
                  if network_ready_retries < network_ready_retries_max then
                    ...
                    @logger.warn(I18n.t("vagrant_fusion.waiting_for_ssh, retrying"))
                  else
                    raise e
                  end
                end
            ...
            # Ready and booted!
            env[:ui].info(I18n.t("vagrant_fusion.ready"))
          end
        ...
        def allows_ssh_port?(env, test_sec_groups, is_vpc)
          ...
          test_sec_groups = [ "default" ] if test_sec_groups.empty? # Fusion default security group
          # filter groups by name or group_id (vpc)
          groups = test_sec_groups.map do |tsg|
            env[:fusion_compute].security_groups.all.select { |sg| tsg == (is_vpc ? sg.group_id : sg.name) }
          end.flatten
          # filter TCP rules
        ...
        def do_elastic_ip(env, domain, server, elastic_ip)
          if elastic_ip =~ /\d+\.\d+\.\d+\.\d+/
            begin
              address = env[:fusion_compute].addresses.get(elastic_ip)
            rescue
            ...
            begin
              allocation = env[:fusion_compute].allocate_address(domain)
            rescue
          ...
          if domain == 'vpc'
            ...
            if address
              association = env[:fusion_compute].associate_address(server.id, nil, nil, address.allocation_id)
            else
              association = env[:fusion_compute].associate_address(server.id, nil, nil, allocation.body['allocationId'])
            ...
            if address
              association = env[:fusion_compute].associate_address(server.id, address.public_ip)
            else
              association = env[:fusion_compute].associate_address(server.id, allocation.body['publicIp'])
