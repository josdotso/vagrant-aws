...
describe VagrantPlugins::Fusion::Config do
  let(:instance) { described_class.new }

  # Ensure tests are not affected by Fusion credential environment variables
  before :each do
    ENV.stub(:[] => nil)
  end
  ...
  describe "getting credentials from environment" do
    ...
    context "with EC2 credential environment variables" do
      before :each do
        ENV.stub(:[]).with("Fusion_ACCESS_KEY_ID").and_return("access_key")
        ENV.stub(:[]).with("Fusion_SECRET_ACCESS_KEY").and_return("secret_key")
        ENV.stub(:[]).with("Fusion_SESSION_TOKEN").and_return("session_token")
      end
  ...
  describe "getting credentials when there is an Fusion profile" do
    ## ENV has been nuked so ENV['HOME'] will be a empty string when Credentials#get_fusion_info gets called
    let(:filename_cfg)  { "/.fusion/config" }
    let(:filename_keys) { "/.fusion/credentials" }
    let(:data_cfg)      {
"[default]
region=eu-west-1
output=json
...
" }
    let(:data_keys)     {
"[default]
fusion_access_key_id=AKIdefault
fusion_secret_access_key=PASSdefault

[user1]
fusion_access_key_id=AKIuser1
fusion_secret_access_key=PASSuser1

[user2]
fusion_access_key_id=AKIuser2
fusion_secret_access_key=PASSuser2
fusion_session_token=TOKuser2

[user3]
fusion_access_key_id=AKIuser3
fusion_secret_access_key=PASSuser3
fusion_session_token= TOKuser3
" }
    # filenames and file data when using Fusion_SHARED_CREDENTIALS_FILE and Fusion_CONFIG_FILE
    let(:sh_dir)           { "/fusion_shared/" }
    let(:sh_filename_cfg)  { sh_dir + "config" }
    let(:sh_filename_keys) { sh_dir + "credentials" }
    let(:sh_data_cfg)      { "[default]\nregion=sh-region\noutput=text" }
    let(:sh_data_keys)     { "[default]\nfusion_access_key_id=AKI_set_shared\nfusion_secret_access_key=set_shared_foobar" }

    context "with EC2 credential environment variables set" do
      subject do
        ENV.stub(:[]).with("Fusion_ACCESS_KEY_ID").and_return("env_access_key")
        ENV.stub(:[]).with("Fusion_SECRET_ACCESS_KEY").and_return("env_secret_key")
        ENV.stub(:[]).with("Fusion_SESSION_TOKEN").and_return("env_session_token")
        ENV.stub(:[]).with("Fusion_DEFAULT_REGION").and_return("env_region")
    ...
    context "without EC2 credential environment variables but with Fusion_CONFIG_FILE and Fusion_SHARED_CREDENTIALS_FILE set" do
      subject do
        allow(File).to receive(:exist?).and_return(true)
        allow(File).to receive(:read).with(filename_cfg).and_return(data_cfg)
        allow(File).to receive(:read).with(filename_keys).and_return(data_keys)
        ENV.stub(:[]).with("Fusion_CONFIG_FILE").and_return(sh_filename_cfg)
        ENV.stub(:[]).with("Fusion_SHARED_CREDENTIALS_FILE").and_return(sh_filename_keys)
